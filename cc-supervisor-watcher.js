...
var STATE_DIR = join(homedir(), ".cc-supervisor");
var ID_FILE = join(STATE_DIR, "install_id");
...
// src/config.ts
function deriveSnapshotUrl(ingestUrl) {
  return ingestUrl.replace(/\/api\/ingest$/, "/api/usage-snapshot");
}
function readEnv() {
  const ingestUrl = process.env.INGEST_URL;
  const configUrl = process.env.CONFIG_URL;
  const token = process.env.INGEST_TOKEN;
  if (!ingestUrl || !configUrl || !token) {
    throw new Error("Missing INGEST_URL/CONFIG_URL/INGEST_TOKEN env");
  }
  const snapshotUrl = process.env.SNAPSHOT_URL ?? deriveSnapshotUrl(ingestUrl);
  return { ingestUrl, configUrl, snapshotUrl, token };
}
async function fetchConfig(env2, installId2) {
  const url = `${env2.configUrl}/${installId2}`;
  const res = await fetch(url, { headers: { authorization: `Bearer ${env2.token}` } });
  if (!res.ok) throw new Error(`config fetch failed: ${res.status}`);
  return await res.json();
}

// src/ingest.ts
async function postBatch(env2, installId2, hostname3, records) {
  const body = { install_id: installId2, hostname: hostname3, records };
  const res = await fetch(env2.ingestUrl, {
    method: "POST",
    headers: { authorization: `Bearer ${env2.token}`, "content-type": "application/json" },
...
async function postUsageSnapshot(env2, installId2, hostname3, profile, rateLimitTier, subscriptionType, usage) {
  const body = {
  ...
  const res = await fetch(env2.snapshotUrl, {
    method: "POST",
    headers: { authorization: `Bearer ${env2.token}`, "content-type": "application/json" },
    body: JSON.stringify(body)
  });
...
function normalizeModelId(raw) {
  if (!raw) return null;
  const noDate = raw.replace(/-(\d{8})$/, "");
  return noDate.replace(/(\d)-(\d)/g, "$1.$2");
}
...
function expandHome(p) {
  ...
  if (p.startsWith("~/")) return homedir2() + p.slice(1);
  return p;
}
function matchesPrefix(cwd, prefixes) {
  if (prefixes.length === 0) return false;
  const norm = cwd.replace(/\/+$/, "") + "/";
  for (const raw of prefixes) {
    const p = expandHome(raw).replace(/\/+$/, "") + "/";
...
// ../../packages/shared/src/eval-schema.ts
var evaluationSchema = external_exports.object({
  clarity: external_exports.number().int().min(1).max(5),
  specificity: external_exports.number().int().min(1).max(5),
  context_quality: external_exports.number().int().min(1).max(5),
  actionability: external_exports.number().int().min(1).max(5),
  overall: external_exports.number().int().min(1).max(5),
  rationale: external_exports.string().min(1).max(400),
  suggested_rewrite: external_exports.string().max(500).nullable()
});
...
function gitProjectKey(cwd) {
  ...
  if (!key) {
    try {
      ...
      if (top) key = `local:${basename3(top)}`;
    } catch {
    }
  }
  if (!key) {
    key = `other:${basename3(cwd)}`;
  }
  projectKeyCache.set(cwd, key);
  return key;
}
function subagentParentSessionId(filePath) {
  const m = filePath.match(/\/([0-9a-f-]{36})\/subagents\/agent-[^/]+\.jsonl$/i);
  return m?.[1] ?? null;
}
function parseLines(text, ctx) {
  const records = [];
  const parentSessionId = subagentParentSessionId(ctx.filePath);
  const isSubagent = parentSessionId !== null;
  const lines = text.split("\n");
  let lineNo = ctx.startLineNo;
  for (const raw of lines) {
    ...
    const cwd = entry.cwd ?? decodeProjectFolder(basename3(dirname4(ctx.filePath)));
    ...
    records.push({
      ...
      text_sha256: sha256(m.text),
      input_tokens: m.usage?.input ?? null,
      output_tokens: m.usage?.output ?? null,
      cache_creation_tokens: m.usage?.cacheCreation ?? null,
...
function readNew(filePath) {
  ...
  try {
    const len = stat4.size - prev;
    const buf = Buffer.alloc(len);
    readSync(fd, buf, 0, len, prev);
    let startLine = 0;
    if (prev > 0) {
      const headerBuf = Buffer.alloc(prev);
      readSync(fd, headerBuf, 0, prev, 0);
      for (const b of headerBuf) if (b === 10) startLine += 1;
    }
    return { text: buf.toString("utf8"), newOffset: stat4.size, startLine };
  } finally {
    closeSync(fd);
  }
}
...
function isAllowed(filePath, allowed2, monitorAll) {
  if (monitorAll) return true;
  const cwd = getCwdFromFile(filePath);
  if (!cwd) return false;
  return matchesPrefix(cwd, allowed2);
}
function startWatch(cfg) {
  const w = watch(PROJECTS_DIR, {
    ignoreInitial: false,
    persistent: true
  });
  const handle = async (filePath) => {
    if (!filePath.endsWith(".jsonl")) return;
    if (!isAllowed(filePath, cfg.allowedPrefixes, cfg.monitorAllPaths)) return;
    const chunk = readNew(filePath);
    if (!chunk) return;
    ...
    await cfg.onRecords(records);
    setOffset(filePath, chunk.newOffset);
  };
  let scanned = 0;
  let allowed2 = 0;
  w.on("ready", () => console.log(`[watch] ready, scanned=${scanned} allowed=${allowed2}`));
  w.on("error", (e) => console.error("[watch] error:", e));
  w.on("add", (p) => {
    if (p.endsWith(".jsonl")) {
      scanned += 1;
      if (isAllowed(p, cfg.allowedPrefixes, cfg.monitorAllPaths)) {
        allowed2 += 1;
  ...
  w.on("change", (p) => {
    if (p.endsWith(".jsonl") && isAllowed(p, cfg.allowedPrefixes, cfg.monitorAllPaths)) {
...
function readClaudeCredentials() {
  try {
    const raw = execFileSync2("security", ["find-generic-password", "-s", "Claude Code-credentials", "-w"], {
      encoding: "utf8",
      stdio: ["ignore", "pipe", "ignore"]
    }).trim();
    if (!raw) return null;
    const json = JSON.parse(raw);
    const c = json.claudeAiOauth;
    if (!c?.accessToken || !c.refreshToken || !c.expiresAt) return null;
    return {
...
function resets(w) {
  if (!w || typeof w.resets_at !== "string") return null;
  return w.resets_at;
}
async function fetchProfile(token) {
  ...
  return {
    ...
    has_claude_max: !!data.account.has_claude_max,
    has_claude_pro: !!data.account.has_claude_pro,
    organization_uuid: data.organization?.uuid ?? null
  };
}
async function fetchUsage(token) {
  ...
  return {
    ...
    five_hour_pct: pct(data.five_hour),
    five_hour_resets_at: resets(data.five_hour),
    seven_day_pct: pct(data.seven_day),
    seven_day_resets_at: resets(data.seven_day),
    seven_day_opus_pct: pct(data.seven_day_opus),
    seven_day_opus_resets_at: resets(data.seven_day_opus),
    seven_day_sonnet_pct: pct(data.seven_day_sonnet),
    seven_day_sonnet_resets_at: resets(data.seven_day_sonnet),
    // Haiku window is published as "seven_day_omelette" in the API response.
    seven_day_haiku_pct: pct(data.seven_day_omelette),
    seven_day_haiku_resets_at: resets(data.seven_day_omelette)
  };
}
...
console.log(`[cc-supervisor] install_id=${installId} host=${hostname2}`);
var allowed = [];
...
async function flush() {
  ...
  const res = await postBatch(env, installId, hostname2, batch);
  if (!res.ok) {
    console.error("[cc-supervisor] ingest failed:", res.error);
    queue.unshift(...batch);
    setTimeout(flush, 3e4);
    return;
  }
  console.log(`[cc-supervisor] ingested ${res.inserted ?? batch.length}`);
...
async function refreshConfig() {
  try {
    ...
    console.error("[cc-supervisor] config refresh failed", e);
  }
}
async function snapshotUsage() {
  ...
  if (!creds) return;
  try {
    const fresh = await fetchProfile(creds.accessToken);
    if (!cachedProfile || cachedProfile.uuid !== fresh.uuid) {
      console.log(`[cc-supervisor] account=${fresh.email ?? fresh.uuid} sub=${creds.subscriptionType ?? "\u2014"}`);
    }
    cachedProfile = fresh;
    if (allowedAccountIds.size > 0 && !allowedAccountIds.has(cachedProfile.uuid)) {
      console.log(
        `[cc-supervisor] paused \u2014 active account ${cachedProfile.uuid.slice(0, 8)} not in company allowlist`
      );
      return;
    }
    const usage = await fetchUsage(creds.accessToken);
    const res = await postUsageSnapshot(
      env,
      installId,
      hostname2,
      cachedProfile,
    ...
    if (!res.ok) {
      console.error("[cc-supervisor] usage snapshot failed:", res.error);
      return;
    }
    console.log(
      `[cc-supervisor] usage 5h=${usage.five_hour_pct ?? "\u2014"}% 7d=${usage.seven_day_pct ?? "\u2014"}% opus=${usage.seven_day_opus_pct ?? "\u2014"}% sonnet=${usage.seven_day_sonnet_pct ?? "\u2014"}% haiku=${usage.seven_day_haiku_pct ?? "\u2014"}%`
    );
  } catch (e) {
    console.error("[cc-supervisor] usage snapshot error", e);
  }
}
void refreshConfig();
